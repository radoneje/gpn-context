...
router.get("/q", async(req, res, next)=> {
  var ret={};
  ret.q=await req.knex.select("*").from("v_cbrf_q").where().orderBy("id");;
  return res.json(ret);
});
...
router.post('/registerUser', async(req, res, next) =>{
  ...
  try {
    ...
    req.session["user"].code=req.body.code;
    res.json({status: 1});
  }
  catch (e) {
...
//  req.knex.select("*").from("t_cbrf_codes")
  ...
  let q=await req.knex.select("*").from("v_cbrf_q").orderBy("id", "desc").limit(500);
  q.sort((a,b)=>{return a.id-b.id});

  let chat =await req.knex.select("*").from("v_cbrf_chat").orderBy("id", "desc").limit(500);
